...
int testfile_id = 1;
int childpidzerocnt = 0;
int timeout_handle_cnt = 0;
int iscrash = 0;
...
struct queue_entry {
  ...
  u32 bitmap_size,                    /* Number of bits set in bitmap     */
      exec_cksum;                     /* Checksum of the execution trace  执行跟踪的校验和 */  
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  struct queue_entry* q = ck_alloc(sizeof(struct queue_entry));  // 通过ck_alloc分配一个 queue_entry 结构体，并进行初始化
...
EXP_ST void init_forkserver(char** argv) {
  ...
  if (pipe(st_pipe) || pipe(ctl_pipe)) PFATAL("pipe() failed"); // 检查 st_pipe 和ctl_pipe，在父子进程间进行管道通信，一个用于传递状态，一个用于传递命令

// 创建子进程后，子进程会执行主函数中fork之后的代码，通过pid的大小来区分执行
  forksrv_pid = fork();  // fork进程出一个子进程 
                         // 如果fork成功，则现在有父子两个进程
  ...
  if (!forksrv_pid) {  // 子进程执行
   ...
   // 设置可以打开的最大的文件描述符的数量
    ...
    // 创建守护进程,也就是说将子进程完全独立出来，子进程完全独立运行
    // 创建一个新会话，并担任该会话组的组长
    // 调用setsid()的目的，让进程摆脱原会话、原进程组、原终端的控制
    // 之所以脱离终端是为了避免进程被任何终端产生的信息打断，其在执行过程中的信息也不在任何终端上显示
    setsid();
    
    // 文件描述符0表示标准输入、1表示标准输出、2表示标准错误
    /*
     重定向标准输出：
     dup2(dev_null_fd, 1); 将标准输出重定向到 /dev/null。这意味着所有写入标准输出的内容都会被丢弃，不会显示在终端上。
     重定向标准错误：
     dup2(dev_null_fd, 2); 将标准错误输出重定向到 /dev/null。这意味着所有写入标准错误的内容也会被丢弃。
    */
    dup2(dev_null_fd, 1);
    dup2(dev_null_fd, 2);
    ...
    if (out_file) {
      dup2(dev_null_fd, 0);  //如果尝试从/dev/null读取内容，什么都读不到
    } else {
      dup2(out_fd, 0);
      close(out_fd);
    }
    ...
    // FORKSRV_FD默认为198, 读管道为198,写管道为199
...
// 带参数执行目标程序，报错才返回
// execv()函数执行后永远不会返回(除非出错)，且运行程序会替换当前进程，pid不变
...
//   printf("%d\n", getpid());
  ...
  if (dumb_mode == 1 || no_forkserver) {  // 如果是dumb_mode模式且没有fork server
    ...
    printf("dumb_mode == 1 || no_forkserver, child_pid\n");
    ...
    // printf("run taget : pid = %d\n", getpid());

    s32 res;

    /* In non-dumb mode, we have the fork server up and running, so simply
       tell it to have at it, and then read back PID. */
       // 如果并不是处在dumb_mode模式，说明fork server已经启动了，我们只需要进行控制管道的写和状态管道的读即可


    // printf("forkserver mode\n");
  ...
  it.it_value.tv_usec = (timeout % 1000) * 1000;    //表示经过timeout时间后将启动定时器
  ...
  //   if (trace_bits[i]>0)
  //   {
  //     printf("before:t[%d]=%d,",i,trace_bits[i]);
  //   }
...
/* Calibrate a new test case. This is done when processing the input directory
   ...
   该函数同样为AFL的一个关键函数，用于新测试用例的校准，在处理输入目录时执行，以便在早期就发现有问题的测试用例，并且在发现新路径时，评估新发现的测试用例的是否可变。
   
   该函数在 perform_dry_run，save_if_interesting，fuzz_one，pilot_fuzzing，core_fuzzing函数中均有调用。该函数主要用途是初始化并启动fork server，多次运行测试用例，并用 update_bitmap_score 进行初始的byte排序
   
    */

static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
                         u32 handicap, u8 from_queue) {
  // printf("========calibrate case===========\n");
  ...
  if (!from_queue || resuming_fuzz)  // 如果from_queue为0（表示case不是来自queue）或者resuming_fuzz为1（表示处于resuming sessions）
  ...
  if (dumb_mode != 1 && !no_forkserver && !forksrv_pid) // 没有运行在dumb_mode，没有禁用fork server，forksrv_pid为0时，启动fork server
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {  // 开始执行 calibration stage，总计执行 stage_max 轮
    ...
    if (!dumb_mode && !stage_cur && !count_bytes(trace_bits)) { // 如果 calibration stage第一次运行，且不在dumb_mode，共享内存中没有任何路径 
      fault = FAULT_NOINST;
    ...
    if (q->exec_cksum != cksum) { // 将当前trace_bits哈希后与前一个的hash比较，如果不相等表示trace_bits发生了变化，即可判断此次变异是否带来了新路径
      ...
      if (q->exec_cksum) {
        ...
        for (i = 0; i < MAP_SIZE; i++) { // 从0到MAP_SIZE进行遍历， first_trace[i] != trace_bits[i]，表示发现了可变queue
...
static void perform_dry_run(char** argv) {
  ...
  u32 cal_failures = 0;
  u8* skip_crashes = getenv("AFL_SKIP_CRASHES");  // 读取环境变量
  ...
  while (q) {     // 遍历队列
    ...
    u8* fn = strrchr(q->fname, '/') + 1;  //打开q->fname，读取到分配的内存中
    ...
    switch (res) {       // 判断res的值
      ...
      case FAULT_NOINST:  // 测试用例运行没有插桩信息
...
keep_as_crash:
      ...
      /* This is handled in a manner roughly similar to timeouts,
         ...
         cases.
         这是以与超时大致相似的方式处理的，除了略有不同的限制并且不需要重新运行测试案例。
         */
...
#define SP20    SP10 SP10
  ...
  SAYF(bV bSTOP "        run time11: " cRST "%-34s " bSTG bV bSTOP
...
void split_files(char* filename){
        s32 source = open(filename, O_RDWR, 0766);

        char number[20];
   
        sprintf(number, "%d", testfile_id);

        s32 dest = open(strcat(filename, number), O_CREAT | O_RDWR, 0766);
        ...
        char *chunk = (char *) malloc(2 * sizeof(char));
        int k;
        do{
           memset(chunk, 0, sizeof(chunk));
           k = read(source, chunk, 1);
           if ((k <= 0))
              break;
...
int copy_by_char(const char *dest_file_name, const char *src_file_name) {
    ...
    FILE *source = fopen(src_file_name,"r");
    if(dest == NULL) {
      char *str1 = "dest error";
      printf("dest error:");
    ...
    fclose(dest);
    // 清空上个窗口的path和case
    FILE *source1 = fopen(src_file_name,"w");
    if(source1 == NULL) {
      printf("source=%s\n",src_file_name);
        perror("source1 error");
        return -1;
    }
    fclose(source1);
    return cnt;
...
int main(int argc, char** argv) {
  ...
  while (1) {
    ...
    if (!queue_cur) {  // 如果queue_cur为空（所有queue都被执行完一轮）
      ...
      if (sync_id && queue_cycle == 1 && getenv("AFL_IMPORT_FIRST"))
        sync_fuzzers(use_argv);   // 对queue_cur进行一次测试 
    ...
    if (!stop_soon && sync_id && !skipped_fuzz) {
      ...
      if (!(sync_interval_cnt++ % SYNC_INTERVAL))    // 如果skipped_fuzz为0且存在sync_id，表示要进行一次sync
