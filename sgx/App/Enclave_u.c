...
static sgx_status_t SGX_CDECL Enclave_ocall_get_time(void* pms)
{
	...
	ocall_get_time(ms->ms_time);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_get_target_pid(void* pms)
{
	ms_ocall_get_target_pid_t* ms = SGX_CAST(ms_ocall_get_target_pid_t*, pms);
	ms->ms_retval = ocall_get_target_pid(ms->ms_afl_pid, ms->ms_target_pid);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_pause_fuzzing(void* pms)
{
	ms_ocall_pause_fuzzing_t* ms = SGX_CAST(ms_ocall_pause_fuzzing_t*, pms);
	ocall_pause_fuzzing(ms->ms_target_pid, ms->ms_forkserver_pid);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_resume_fuzzing(void* pms)
{
	ms_ocall_resume_fuzzing_t* ms = SGX_CAST(ms_ocall_resume_fuzzing_t*, pms);
	ocall_resume_fuzzing(ms->ms_pid, ms->ms_forkserver_pid);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_get_fuzz_worker_pid(void* pms)
{
	ms_ocall_get_fuzz_worker_pid_t* ms = SGX_CAST(ms_ocall_get_fuzz_worker_pid_t*, pms);
	ms->ms_retval = ocall_get_fuzz_worker_pid(ms->ms_target_pid, ms->ms_fuzz_worker_pid);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_check_afl(void* pms)
{
	ms_ocall_check_afl_t* ms = SGX_CAST(ms_ocall_check_afl_t*, pms);
	ms->ms_retval = ocall_check_afl();

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_get_afl_pid(void* pms)
{
	ms_ocall_get_afl_pid_t* ms = SGX_CAST(ms_ocall_get_afl_pid_t*, pms);
	ms->ms_retval = ocall_get_afl_pid(ms->ms_pid);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_notify_java(void* pms)
{
...
static sgx_status_t SGX_CDECL Enclave_ocall_check_java(void* pms)
{
	ms_ocall_check_java_t* ms = SGX_CAST(ms_ocall_check_java_t*, pms);
	ms->ms_retval = ocall_check_java(ms->ms_isEqual);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_write_shm(void* pms)
{
	if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
	ocall_write_shm();
	return SGX_SUCCESS;
...
static sgx_status_t SGX_CDECL Enclave_create_shared_memory(void* pms)
{
	ms_create_shared_memory_t* ms = SGX_CAST(ms_create_shared_memory_t*, pms);
	create_shared_memory(ms->ms_shm_ptr);
...
static const struct {
	...
	22,
	{
		...
		(void*)Enclave_ocall_get_target_pid,
		(void*)Enclave_ocall_pause_fuzzing,
		(void*)Enclave_ocall_resume_fuzzing,
		(void*)Enclave_ocall_get_fuzz_worker_pid,
		(void*)Enclave_ocall_check_afl,
		(void*)Enclave_ocall_get_afl_pid,
		(void*)Enclave_ocall_notify_java,
		(void*)Enclave_ocall_check_java,
		(void*)Enclave_ocall_write_shm,
		(void*)Enclave_create_shared_memory,
